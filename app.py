...
_rename_llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash")
...
with st.sidebar:
    ...
    if st.button("Add Files", use_container_width=True, key="btn_add_files"):
        if uploaded_files:
            ...
            upload_results = []  # collect messages to show AFTER rerun

            with st.spinner("Processing files…"):
                for f in uploaded_files:
                    if f.name in st.session_state[processed_key]:
                        st.info(f"Already loaded: {f.name}")
                        upload_results.append(("info", f"Already loaded: {f.name}"))
                        continue
                    ...
                    try:
                        ...
                        upload_results.append(("success", f"Added: {f.name}"))
                    except Exception as e:
                        st.error(f"Failed: {f.name} — {e}")
                        upload_results.append(("error", f"Failed: {f.name} — {type(e).__name__}: {e}"))
                    finally:
                        if tmp_path:
    ...
    # Show results from the PREVIOUS run, right after the button/uploader
    if "upload_results" in st.session_state:
        for level, msg in st.session_state["upload_results"]:
            getattr(st, level)(msg)
        del st.session_state["upload_results"]   # clear so it doesn't repeat forever
            ...
            try:
                for chunk, metadata in graph.stream(input_state, config, stream_mode="messages"):
                    if (
                        ...
                        and isinstance(chunk, AIMessageChunk)   # <-- only true streaming deltas
