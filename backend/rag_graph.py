...
llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash")  # or gemini-2.5-pro
...
def web_search(
    ...
    try:
        ...
        return [ToolMessage(
            content=f"Web search is unavailable right now ({type(e).__name__}). Continuing without it.",
...
def verify_claim_node(state: RAGState) -> dict:
    ...
    try:
        ...
        return {
            "claim_verdict": "Could not verify this claim right now — the search service is unavailable.",
            "claim_source": None,
            "superseding_papers": [],
        }
